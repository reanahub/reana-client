...
def list_all_jwt_workflows():
    ...
    try:
        ...
        display_message("Cannot read access token", msg_type="error")
        sys.exit(1)
    if not server_url:
        display_message("REANA_SERVER_URL is not set", msg_type="error")
        sys.exit(1)

    # Fetch workflows
    url = f"{server_url.rstrip('/')}/api/workflows"
    ...
    try:
        ...
        display_message(f"Error fetching workflows: {e}", msg_type="error")
        sys.exit(1)
