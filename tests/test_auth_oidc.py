...
def test_discarded_refresh_revocation_does_not_block_other_servers(
    tmp_path, monkeypatch
):
    """A slow discarded-token revocation must leave the credential store usable."""
    import threading
    ...
    finish_revocation = threading.Event()
    revoke_calls = []

    def fake_post(url, data, timeout, allow_redirects, verify):
        if url == "https://issuer.example.org/token":
            # Logout wins while refresh is in flight, forcing the returned
            # tokens to be discarded rather than saved.
            storage.clear_token_material(server_a)
            return MockResponse(
                {
        ...
        if url == "https://issuer.example.org/revoke":
            ...
            assert finish_revocation.wait(timeout=10)
            return MockResponse({})
        raise AssertionError(f"unexpected POST to {url}")
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        refresh = executor.submit(oidc.refresh_credentials, server_a)
        try:
            ...
            # Exercise the real credential-store lock from another thread
            # while the issuer's revocation response is still blocked.
            assert update.result(timeout=5)["access_token"] == "b-access"
    ...
    entry = get_server_entry(server_a)
    assert "access_token" not in entry
    assert "refresh_token" not in entry
    assert get_server_entry(server_b)["refresh_token"] == "b-refresh"
    assert load_config()["active_server"] == server_b
    assert revoke_calls == [
        {
            "client_id": "reana-cli",
            "token": "discarded-refresh",
...
def test_tls_verify_defaults_to_enabled(monkeypatch):
    ...
    monkeypatch.delenv(config.TLS_VERIFY_ENV, raising=False)
...
def test_server_tls_override_keeps_identity_provider_verification(
    ...
    def record_request(url, **kwargs):
        requests.append((url, kwargs["verify"]))
        return MockResponse(dict(METADATA))

    monkeypatch.setattr(oidc.requests, "get", record_request)
    monkeypatch.setattr(oidc.requests, "post", record_request)
    metadata = oidc.discover("https://reana.example.org")
    pkce = oidc.generate_pkce_pair()
    oidc._start_device_authorization("https://reana.example.org", metadata, pkce)
    oidc._exchange_authorization_code(
        "https://reana.example.org", metadata, "code", pkce, "http://localhost"
    )

    assert requests == [
        (
            "https://reana.example.org/api/.well-known/openid-configuration",
            ca_bundle or False,
        ),
        (metadata["device_authorization_endpoint"], ca_bundle or True),
        (metadata["token_endpoint"], ca_bundle or True),
    ]
